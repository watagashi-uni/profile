<template lang="pug">
  div
    v-list.py-0(dense)
      ...
      v-list-item
        v-list-item-avatar
          v-img(:src="`${$assets()}/thumbnail/chara_rip/${$root.cards[profile.userDecks[0].leader].assetbundleName}_${profile.userCards.find(card => card.cardId == profile.userDecks[0].leader).defaultImage == 'special_training' ? 'after_training' : 'normal'}.png`")
        v-list-item-content
      ...
      Divider(inset=16)
      .d-flex.pa-2
        div(style="width: 50%"): v-img(v-if="profile.userProfile.honorId1", contain, height=40, :src="`${$assets()}/honor/${$root.honors[profile.userProfile.honorId1].assetbundleName}_rip/degree_main.png`")
        div(style="width: 25%"): v-img(v-if="profile.userProfile.honorId2", contain, height=40, :src="`${$assets()}/honor/${$root.honors[profile.userProfile.honorId2].assetbundleName}_rip/degree_sub.png`")
        div(style="width: 25%"): v-img(v-if="profile.userProfile.honorId3", contain, height=40, :src="`${$assets()}/honor/${$root.honors[profile.userProfile.honorId3].assetbundleName}_rip/degree_sub.png`")
      Divider(inset=16)
      v-list-item
